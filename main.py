...
with st.sidebar:
    ...
    available_years = db_manager.get_user_expense_years(user['id'])
...
with tab1:
    ...
    if view_mode == "Monthly View" and selected_month is not None:
        # --- FIX APPLIED HERE ---
        # Separated the variable assignments into two lines to fix the ValueError.
    ...
    if dashboard_df.empty:
        ...
        with st.expander("View Raw Data for Selected Period"):
            st.dataframe(
                dashboard_df[['date', 'category', 'title', 'amount']].rename(
                    columns={'date':'Date', 'category':'Category', 'title':'Title', 'amount':'Amount'}
            ...
            if voice_result['status'] == 'success':
                ...
                with st.spinner("🤖 AI is processing..."):
                    ...
                    if expense_data:
                        expense_data['user_id'] = user['id']
                        db_manager.save_expense(expense_data)
                        st.success(f"Saved: {expense_data['title']} (${expense_data['amount']})")
                        st.rerun()
