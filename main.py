...
# --- ENHANCEMENT: Import the microphone availability check ---
...
def init_components():
    ...
    return {
        'db': db_manager, 'gemini': GeminiClient(config.gemini_api_key, db_manager),
        'voice': VoiceProcessor(), 'auth': AuthManager(db_manager),
        'export': ExportManager(), 'ui': UIComponents()
    }
    ...
    with login_tab:
        with st.form("login_form"):
            ...
            if st.form_submit_button("Login"):
                if auth_manager.login(username, password): st.rerun()
    ...
    with register_tab:
        with st.form("register_form"):
            ...
            if st.form_submit_button("Register"):
                if auth_manager.register(new_username, new_password): st.success("Registration successful! Please login.")
...
with tab1:
    ...
    if view_mode == "Monthly View" and selected_month is not None:
        ...
        st.subheader(f"Showing Overview for {calendar.month_name[selected_month]} {selected_year}")
    ...
    st.divider()
    dashboard_df = db_manager.get_filtered_expenses(user_id=user['id'], start_date=start_date, end_date=end_date, categories=selected_categories, amount_range=(0, float('inf')))
    if dashboard_df.empty: st.info("No expenses found for the selected period and categories.")
    else:
        ...
        c1, c2, c3 = st.columns(3); c1.metric("💸 Total Expenses", f"${total_expense:,.2f}"); c2.metric("📊 Avg. Transaction", f"${avg_expense:,.2f}"); c3.metric("🏆 Top Category", top_category); st.divider()
        c1, c2 = st.columns(2)
        with c1:
            fig_pie = px.pie(dashboard_df.groupby('category')['amount'].sum(), values='amount', names=dashboard_df.groupby('category')['amount'].sum().index, title="Expense Distribution")
            st.plotly_chart(fig_pie, use_container_width=True)
        with c2:
            if view_mode == "Monthly View":
                fig_trend = px.line(dashboard_df.groupby(dashboard_df['date'].dt.date)['amount'].sum().reset_index(), x='date', y='amount', title="Daily Spending Trend", markers=True)
            else:
                dashboard_df['month'] = dashboard_df['date'].dt.strftime('%b')
                fig_trend = px.bar(dashboard_df.groupby('month')['amount'].sum().reindex(calendar.month_abbr[1:]).fillna(0).reset_index(), x='month', y='amount', title="Monthly Spending Trend")
            st.plotly_chart(fig_trend, use_container_width=True)
        with st.expander("View Raw Data for Selected Period"):
            st.dataframe(dashboard_df[['date', 'category', 'title', 'amount']].rename(columns={'date':'Date', 'category':'Category', 'title':'Title', 'amount':'Amount'}).reset_index(drop=True))
...
with tab2:
        ...
        with st.form("smart_add_form", clear_on_submit=True):
            ...
            if st.form_submit_button("✨ Parse and Add") and text_input:
                with st.spinner("🤖 AI is processing..."):
                    ...
                    if expense_data:
                        expense_data['user_id'] = user['id']; db_manager.save_expense(expense_data);
                        st.success("Saved!"); st.rerun()
                    else: st.error("AI could not parse the input.")
    with add_tab2:
        # --- ENHANCEMENT: Conditionally render the voice UI ---
    ...
    with add_tab3:
        with st.form("manual_add_form", clear_on_submit=True):
            ...
            title, amount = st.text_input("Title/Description"), st.number_input("Amount", 0.01, format="%.2f")
            date, notes = st.date_input("Date", datetime.now().date()), st.text_area("Notes (Optional)")
            if st.form_submit_button("💾 Save Manually") and title and amount and category:
                ...
                db_manager.save_expense(expense_data); st.success("Saved!"); st.rerun()
...
with tab3:
    ...
    if export_period == "Filtered Range":
        c1, c2 = st.columns(2); export_start_date = c1.date_input("From", datetime.now().date() - timedelta(days=30), key="export_start"); export_end_date = c2.date_input("To", datetime.now().date(), key="export_end")
    else:
        today = datetime.now().date()
        if export_period == "This Week": export_start_date, export_end_date = today - timedelta(days=today.weekday()), today
        ...
        elif export_period == "This Month": export_start_date, export_end_date = today.replace(day=1), today
        ...
        elif export_period == "This Year": export_start_date, export_end_date = today.replace(month=1, day=1), today
    all_user_categories = db_manager.get_user_categories(user['id'])
    ...
    if export_df.empty: st.warning(f"No data to export for the selected period: **{export_period}**.")
    else:
        st.info(f"Preparing to export **{len(export_df)}** transactions from **{export_start_date}** to **{export_end_date}**.")
        c1, c2, c3 = st.columns(3); file_period_str = export_period.lower().replace(' ', '_')
